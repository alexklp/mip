...
КАЛІБРАЦІЙНЕ УТОЧНЕННЯ ДО КРОКУ 1 І КРОКУ 2 (має вищий пріоритет за будь-яку
...
6. Для подій у великій локації (місто/область/країна) shared_referent_status
   може бути confirmed лише коли, КРІМ самої локації і загального типу події,
   обидві сторони мають ще хоча б одну спільну event-specific ознаку: той самий
   названий об'єкт/ціль, учасник, унікальна дія, конкретний документ/посада,
   специфічна комбінація деталей тощо. Лише "Київ + вибухи/удар" недостатньо.
7. Не підмінюй відсутню спільну ознаку припущенням з правдоподібності. Якщо
   контекст дозволяє, що це два різні інциденти, і немає позитивного доказу
   одного інциденту, shared_referent_status не може бути confirmed.
8. same_fact означає ОДНУ Й ТУ САМУ ПРОПОЗИЦІЮ, а не просто спільну тему чи
   спільну подію. Повинні збігатися фактичний суб'єкт/target, дія або стан,
   об'єкт/результат і суттєві qualifiers. Наприклад, "організація потрапила
   під санкції" і "керівник організації потрапив під санкції" — це РІЗНІ
   факти; за підтвердженої спільної санкційної події це може бути same_event,
   але НЕ same_fact.
9. Не приписуй claim B твердження, яке є лише в title/context або в claim A.
   Перед same_fact окремо перевір, що claim A і claim B взаємно стверджують
   одну й ту саму фактичну пропозицію.
""".strip()
